<template>
    <div>
        <div class="table">
			...
			<div v-if="tableOrderSell.length==0" class="nodata">暂无数据</div>
            <ul class="content" v-else>
                <li v-for="(item,i) in tableOrderSell" :key="i">
...
export default {
    ...
    methods:{
        ...
        getAdv(name) {
			this.$http
				.post(this.host + "/otc/advertise/member", {
                    ...
                    var resp = response.body;
                    if (resp.code == 0) {
                        this.tableOrderSell = resp.data.sell.map(ele=>{
		...
		whichItem(id,i){
			this.tableOrderSell=this.tableOrderSell.map(ele=>{
				ele.Itemclicked=true;
				return ele;
			})
			this.tableOrderSell[i].Itemclicked=false;
			this.getIdAdv(id);
    	},
		closeItem(i){
			this.tableOrderSell[i].Itemclicked=true;
		},
		//获取id广告信息
		getIdAdv(id) {
			this.$http
