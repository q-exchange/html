  ...
  <div class="index-wrap">
    ...
    <div id="cont">
      <div id="adv">
        ...
        <div class="advBtn">
          <button-group>
            <i-button v-for="(item,index) in btnList" :key="index" @click="addClass(index)" :class="{'ivu-btn-primary':index==choseBtn,'ivu-btn-default':index!=choseBtn}">{{item.text}}</i-button>
          </button-group>
        </div>
        <div class="item1" v-show="isitem1">
          <Row type="flex" justify="space-around" class="code-row-bg adv_box">
            <Col span="6" v-for="(item,index) in cardListBuy0" :key="index" class="card-item">
            <Card>
        ...
        <div class="item2" v-show="!isitem1">
          <Row type="flex" justify="space-around" class="code-row-bg adv_box">
            <Col span="6" v-for="(item,index) in cardListBuy1" :key="index" class="card-item">
            <Card>
              <router-link :to="'/tradeInfo?tradeId='+item.advertiseId" class="tread-list trade-item">
                <div class="trade-body">
                  <div class="user-info">
...
.trade-item .trade-body .user-info {
  ...
  padding: 20px 0;
  margin-bottom: 30px;
  border-bottom: @border-width_primary @border-style_dotted #e7e9ed;
}
...
.trade-item .trade-btn {
  ...
  font-size: 16px;
  display: block;
  border: @border-width_primary @border-style_solid #96e1fb;
