      ...
      <div>
        <div class="send-box">
          ...
          <div class="formbox send-form">
            <Form ref="form" :model="form" :rules="ruleValidate" :label-width="90" label-position="left">
              ...
              <FormItem :label="$t('otc.publishad.currency')" prop="rmb">
                <Input v-model="form.rmb" disabled></Input>
              </FormItem>
              ...
              <FormItem :label="$t('otc.publishad.exchangeperiod')" prop="timeLimit" style="padding-top:10px">
              ...
              <FormItem>
                <Button style="background:#357ce1;color:#fff;border:1px solid #357ce1;" long @click="handleSubmit('form')" :disabled="disAllowBtn">{{$t('otc.publishad.submit')}}</Button>
...
export default {
  ...
  computed: {
    ...
    wantstyle(){
      return this.$t("otc.publishad.sellout");
    },
    // 付款方式
    payModeList(){
      let arr=[];
...
/deep/ .my_ad_container .my_ad_container_spin.ivu-spin-fix .ivu-spin-main {
  top: 200px;
}
.my_ad_container {
  padding:80px 10%;
...
.title-box {
  border-bottom:@border-width_primary @border-style_dashed #ccc;
  padding-bottom: 30px;
  text-align: left;
  .title_msg{
    padding:0 24px;
...
.content-wrap {
  background:@content-wrap_bgcolor;
  min-height: 750px;
}
