<template>
    <div class="specail" style="min-height:519px;">
        <div class="header-title">
            <span>{{$t('googleverify._openGoogle')}}</span>
        ...
        <div class="wrapper">
            <div class="qrclass">
                ...
                <div>{{$t('googleverify._downloadAndroid')}}</div>
            </div>
            <div class="qrclass">
                <img src="../../assets/images/goole_IOS.png" alt="">
                <div>{{$t('googleverify._downloadIos')}}</div>
            </div>
            <div class="qrflex">
                  <div id="qrcode"></div>
            <div class="abstract">
                <p>{{$t('googleverify._googleVerifyabs')}}</p>
                <p>
                    <span>{{data.secret}}</span>
                    <span v-clipboard:copy="data.secret" v-clipboard:success="onCopy" v-clipboard:error="onError" class="blue">{{$t('googleverify._copy')}}</span>
                </p>
            </div>
            </div>
        </div>
        <ul>
            <li>{{$t('googleverify._abs1')}}</li>
            <li>{{$t('googleverify._abs2')}}</li>
            <li>{{$t('googleverify._abs3')}}</li>
        </ul>
        <Form :model="formCode" label-position="top" :rules="ruleInline" ref="formCode" class="form">
            <FormItem :label="$t('googleverify._GoogleVerificationCode')" prop="code">
                <Input v-model="formCode.code"></Input>
            </FormItem>
            <FormItem>
                <Button type="primary" style="width:100%;" @click="sure('formCode')">{{$t('googleverify._sure')}}</Button>
            </FormItem>
        </Form>
...
export default {
    ...
    data() {
        return {
            ...
            ruleInline: {
                code: [{ required: true, message: this.$t('googleverify._mustFill'), trigger: 'blur' }]
            }
        }
    },
    created() {
        ...
        sure(name) {
            this.$refs[name].validate(valid => {
                if (valid) {
                    ...
                    this.$Notice.erroe({
                        title: this.$t("common.tip"),
                        desc: this.$t("googleverify._googleCodeRequire"),
                    })
                }
            })
        },
